...
if($privres2->num_rows>0){
    ...
    $user = $_COOKIE["type"];
    $user2 = md5($user);
    $preq = "select * from ".$user2." where identity='$node'";
    $prere = $conn->query($preq);
    if($prere->num_rows>0){
        $prerow = $prere->fetch_assoc();
        if($prerow["rating"]!=0){
            $oval = $prerow["rating"];
            $prere2 = "update ".$user2." set rating='$val' where identity='$node'";
            ...
            $prere3 = "update ".$user2." set rating='$val' where identity='$node'";
        ...
        $finfin = "insert into ".$user2." (datatype,owner,partner,identity,following,rating) values ('disc',0,0,'$node',0,'$val')";
    ...
    $sql = "select * from discs where identity='$node'";
    $result = $conn->query($sql);
    $row = $result->fetch_assoc();
    $dat = $row["rating"]+$val-$oval;
    $sql2 = "update discs set rating='$dat' where identity='$node'";
